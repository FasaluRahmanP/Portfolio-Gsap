...
export default function CombinedPortfolio() {
  ...
  }, [showSplashScreen, slides.length]);

  // Initialize GSAP ScrollTrigger for portfolio
  useEffect(() => {
    if (!showSplashScreen && typeof window !== "undefined") {
      gsap.registerPlugin(ScrollTrigger);
  ...
  // Setup cursor-following arrow animation
  useEffect(() => {
    if (!showSplashScreen && letsTalkRef.current && arrowRef.current) {
      // Initially hide the arrow
      gsap.set(arrowRef.current, { opacity: 0, scale: 0.8 });
